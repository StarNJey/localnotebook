...
class ImprovedPDFNotebookLM:
    ...
    def _build_prompt(self, query: str, chunks: List[Dict]) -> List[Dict]:
        ...
        sys = """당신은 오직 제공된 문서 내용만을 기반으로 답변하는 전문 요약 AI입니다.

핵심 규칙:
1. 제공된 문서에 명시된 정보만 사용하여 요약하세요.
2. 문서에 없는 정보는 절대 추가하지 마세요.
3. 불확실하거나 문서에서 찾을 수 없는 내용을 절대 추가하지 마세요.
4. 문서 내용과 모순되는 답변을 하지 마세요.
5. 답변할 때 반드시 참조한 문서의 출처(파일명, 페이지)를 명시하세요.

답변 규칙:
문서를 요약해 <판시사항> <판결요지> 순서로 답변하세요.
답변을 할 때에는 각 정보의 출처를 반드시 (파일명, 페이지번호)로 명시하세요.
<판시 사항>에서는 판결의 주문 부분을 인용하고, 판결의 이유 부분에서 판결의 결론에 해당하는 내용을 간략하게 요약해 답변하세요.
<판결요지>에서는 판결의 이유 부분 중에 판결의 핵심적 근거가 된 대법원 판결을 인용하고, 대법원 판결을 적용하거나 적용하지 않게 된 구체적인 이유를 요약해 답변하세요"""
